...
// ——— Theme & visited (your original code) ———
...
// ——— Accept Ads ———
function acceptAds() {
    setCookie("ads_consent", "granted", 365);
    gtag('consent', 'update', {
        'ad_storage': 'granted',
...
// ——— Reject Ads ———
function rejectAds() {
    setCookie("ads_consent", "denied", 365);
    hideConsentBanner();
}

// ——— Hide banner (the real fix) ———
function hideConsentBanner() {
    const banner = document.getElementById("consent-banner");
    if (banner) banner.remove();
}

// ——— Load AdSense only once ———
function loadGoogleAds() {
    if (document.querySelector('script[src*="adsbygoogle.js"]')) return; // already loaded

    const script = document.createElement("script");
    script.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-YOUR-PUB-ID";
    script.async = true;
...
if (!["granted", "denied"].includes(getCookie("ads_consent"))) {
    setCookie("ads_consent", "pending", 365);
    // banner will be shown by HTML below
} else if (getCookie("ads_consent") === "granted") {
    hideConsentBanner();
    loadGoogleAds();
} else {
    hideConsentBanner();
