...
                alert("Allow popups for this site to use the auto about:blank feature.");
            }
        } else {
            hasAboutBlankRun = false; 
            alert("Allow popups for this site to use the auto about:blank feature.");
...
// script.js – Only themes + first-time visitor + favorites (perfect for unblocked games)
...
// 2. Theme (dark / light) – remembers forever
const savedTheme = getCookie("theme");
if (savedTheme === "light") {
    document.documentElement.setAttribute("data-theme", "light");
} else {
    // default = dark mode (most unblocked players love it)
    document.documentElement.setAttribute("data-theme", "dark");
    setCookie("theme", "dark", 365);
}

// 3. Favorites system (star button)
function toggleFavorite(gameId) {
    let favs = getCookie("favorites") || "";
    let list = favs ? favs.split(",") : [];
    ...
    if (list.length > 0) {
        setCookie("favorites", list.join(","), 365);
    } else {
        setCookie("favorites", "", -1); // delete cookie
    }
}

// Optional tiny helper so you can see it works
console.log("Cookies ready: theme =", getCookie("theme"), "| favorites =", getCookie("favorites"));